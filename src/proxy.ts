...
const disabledMarketingPrefixes = [
  '/blog',
  '/docs',
  '/about',
  '/contact',
...
export default async function proxy(req: NextRequest) {
  ...
  // Redirect disabled legacy marketing pages to homepage.
  const isDisabledMarketingPage = disabledMarketingPrefixes.some(
    (prefix) =>
      pathnameWithoutLocale === prefix ||
      pathnameWithoutLocale.startsWith(`${prefix}/`)
  );
  if (isDisabledMarketingPage) {
